...
class VisionLinkApp(ctk.CTk):
    def __init__(self):
        ...
        # ==============================================================
        # FIXED ALIGNMENT: All spaces are exactly matched now!
        # ==============================================================
    ...
    def start_camera_feed(self):
        ...
        if is_attendance_active:
            pil_image, _ = self.vision_engine.get_frame(ai_active=True, scan_mode="attendance")
            
            if pil_image and self.current_tab == "attendance" and hasattr(self, 'attendance_camera_screen'):
            ...
            if pil_image and self.current_tab == "energy" and hasattr(self, 'camera_screen'):
            ...
            if not is_ai_active:
                ...
                if not is_any_device_on:
                    if self.last_voice_state != "SLEEP":
                        ...
                        speak_text("Initiating Sleep Mode to save power.")
                    ...
                    if human_present:
                        if self.vision_engine.presence_timer < self.vision_engine.MAX_TIMER:
                            ...
                            self.last_voice_state = "SLEEPING"
                        else:
                            if self.last_voice_state != "DETECTED":
                                self.last_voice_state = "DETECTED"
                                speak_text("Target Locked. Monitoring active.")
    ...
    def update_image_on_label(self, pil_image, label):
        ...
        ratio = min(850 / orig_w, 680 / orig_h)
    ...
    def setup_attendance_frame(self):
        page_title_frame = ctk.CTkFrame(self.frame_attendance, fg_color=self.BG_CARD, corner_radius=10, border_width=1, border_color=self.BORDER_MUTED, height=50, width=400)
        page_title_frame.pack(pady=(0, 20))
        page_title_frame.pack_propagate(False)
        ctk.CTkLabel(page_title_frame, text="  📷  LIVE ATTENDANCE SCANNER", font=ctk.CTkFont(size=20, weight="bold"), text_color=self.ACCENT_CYAN).pack(expand=True)

        control_panel = ctk.CTkFrame(self.frame_attendance, fg_color="transparent")
        control_panel.pack(fill="x", padx=40, pady=(0, 10))
        
        self.attendance_running = False
        self.btn_toggle_attendance = ctk.CTkButton(control_panel, text="▶ START LIVE ATTENDANCE", 
                                                   font=ctk.CTkFont(size=16, weight="bold"), height=45, width=250,
                                                   fg_color=self.NEON_GREEN, text_color="#000000", hover_color=self.ACCENT_CYAN,
                                                   command=self.toggle_attendance)
        self.btn_toggle_attendance.pack(side="left")
        
        self.attendance_status_lbl = ctk.CTkLabel(control_panel, text="STATUS: OFFLINE", font=ctk.CTkFont(size=16, weight="bold"), text_color=self.TEXT_MUTED)
        self.attendance_status_lbl.pack(side="left", padx=20)

        camera_panel = ctk.CTkFrame(self.frame_attendance, fg_color=self.BG_CARD, corner_radius=15, border_width=1, border_color=self.BORDER_MUTED)
        camera_panel.pack(fill="both", expand=True, padx=40, pady=(0, 40))

        self.attendance_camera_screen = ctk.CTkLabel(camera_panel, text="[ Attendance Vision Initializing... ]\nClick 'START LIVE ATTENDANCE' button to begin", 
                                                     font=ctk.CTkFont(size=18), text_color=self.TEXT_MUTED, fg_color="#000000", corner_radius=10)
        self.attendance_camera_screen.pack(fill="both", expand=True, padx=20, pady=20)

    def toggle_attendance(self):
        self.attendance_running = not self.attendance_running
        if self.attendance_running:
            self.btn_toggle_attendance.configure(text="⏹ STOP LIVE ATTENDANCE", fg_color=self.ALERT_RED, text_color=self.TEXT_WHITE)
            self.attendance_status_lbl.configure(text="STATUS: SCANNING LIVE...", text_color=self.NEON_GREEN)
            speak_text("Live Attendance Scanner Activated.")
        else:
            self.btn_toggle_attendance.configure(text="▶ START LIVE ATTENDANCE", fg_color=self.NEON_GREEN, text_color="#000000")
            self.attendance_status_lbl.configure(text="STATUS: OFFLINE", text_color=self.TEXT_MUTED)
            speak_text("Live Attendance Scanner Deactivated.")
