...
class VisionLinkApp(ctk.CTk):
    def __init__(self):
        ...
        self.logo_label = ctk.CTkLabel(self.logo_frame, text="VISIONLINK",
                                       font=ctk.CTkFont(size=22, weight="bold"), text_color=self.TEXT_WHITE)
        self.logo_label.pack(pady=(0, 2))
        self.logo_sub = ctk.CTkLabel(self.logo_frame, text="SMART CLASS ENGINE  v2.0",
                                     font=ctk.CTkFont(size=10, weight="bold"), text_color=self.ACCENT_CYAN)
        self.logo_sub.pack()
    ...
    def show_toast(self, message, toast_type="success"):
        ...
        toast = ctk.CTkFrame(self, fg_color=bg_color, corner_radius=12, 
                            border_width=2, border_color=border_color, width=320, height=45)
        toast.place(relx=0.98, rely=0.02, anchor="ne")
        toast.lift()
        toast.pack_propagate(False)
        
        ctk.CTkLabel(toast, text=f"  {icon}  {message}", 
                    font=ctk.CTkFont(size=13, weight="bold"), 
                    text_color=color).pack(expand=True, padx=10)
    ...
    def create_sidebar_btn(self, text, row, command):
        btn = ctk.CTkButton(self.sidebar_frame, text=text, font=ctk.CTkFont(size=14), height=50, anchor="w",
                            fg_color="transparent", hover_color=self.BTN_HOVER_BG, border_spacing=15,
                            text_color=self.TEXT_MUTED, corner_radius=10, command=command)
    ...
    def setup_records_frame(self):
        ...
        btn_refresh = ctk.CTkButton(controls_bar, text="  🔄  Refresh", font=ctk.CTkFont(size=14, weight="bold"), height=40, width=120,
                                    fg_color=self.ACCENT_CYAN, text_color="#000000", hover_color="#FFFFFF", command=self.load_records_data)
        btn_refresh.pack(side="left", padx=(0, 10))
        
        btn_clear = ctk.CTkButton(controls_bar, text="  🗑  Clear All", font=ctk.CTkFont(size=14, weight="bold"), height=40, width=120,
                                  fg_color="#4C0519", hover_color=self.ALERT_RED, command=self.clear_all_records)
        ...
        for header_text, width_ratio in headers:
            ctk.CTkLabel(header_frame, text=header_text, font=ctk.CTkFont(size=14, weight="bold"), 
                        text_color=self.ACCENT_CYAN, anchor="w").place(relx=current_x, rely=0.5, anchor="w", relwidth=width_ratio)
        ...
        self.records_empty_label = ctk.CTkLabel(self.records_scroll, text="No attendance records found.\nSave a session from Live Attendance to see data here.", 
                                                font=ctk.CTkFont(size=16), text_color=self.TEXT_MUTED)
    ...
    def load_records_data(self):
        ...
        if len(records) == 0:
            empty_lbl = ctk.CTkLabel(self.records_scroll, text="No matching records found.", 
                                     font=ctk.CTkFont(size=16), text_color=self.TEXT_MUTED)
        ...
        for idx, row in enumerate(records):
            ...
            for col_text, width_ratio in cols:
                ...
                ctk.CTkLabel(row_frame, text=col_text, font=ctk.CTkFont(size=13), 
                            text_color=text_color, anchor="w").place(relx=current_x, rely=0.5, anchor="w", relwidth=width_ratio)
                current_x += width_ratio
